	...
	function woodmart_get_number_param( $settings, $value ) {
		...
		<div class="wd-numbers">
			...
			<?php foreach ( $settings['devices'] as $device => $device_settings ) : ?>
				...
				<input type="number" class="wd-number<?php echo esc_attr( $classes ); ?>" min="<?php echo esc_attr( $settings['min'] ); ?>" max="<?php echo esc_attr( $settings['max'] ); ?>" step="<?php echo esc_attr( $settings['step'] ); ?>" data-device="<?php echo esc_attr( $device ); ?>" value="<?php echo esc_attr( $device_settings['value'] ); ?>">
