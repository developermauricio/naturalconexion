...
(function($) {
	...
	ImportBox.prototype.formSubmit = function(e) {
		...
		if ($form.hasClass('form-in-action')) {
			return;
		}
		...
		if ($form.find('#full_import').prop('checked') == true) {
			...
			var ajaxComplete = function() {
				if (!versions[i]) {
					...
					$('.xts-wizard-content-inner').addClass('imported-' + versions.join(' imported-'));
					importBox.mainArea.find('.full-import-box').remove();
					$('.xts-setup-wizard .full-import-box').remove();
				} else {
		...
		if ( 'base' !== version ) {
			$('.xts-wizard-note.xts-success').fadeOut();
		}
		...
		importBox.callImportAJAX(version, function(response) {
			...
			$('.xts-wizard-content-inner').addClass('imported-' + version);
	...
	woodmartAdminModule = (function() {
		...
		var woodmartAdmin = {
			...
			attributesMetaboxes: function() {
				...
				if (!$('body').hasClass('product_page_product_attributes')) {
					return;
				}
				...
				var orderByRow            = $('#attribute_orderby').parent(),
				    ...
				    labelSelectSize       = '<label for="attribute_swatch_size">' + woodmartConfig.attribute_swatch_size_title + '</label>',
				    descriptionSelectSize = '<p class="description">' + woodmartConfig.attribute_swatch_size_description + '</p>',
				    selectSize            = [
				    ...
				    labelShowAttr         = '<label for="attribute_show_on_product">' + woodmartConfig.attribute_show_on_product_title + '</label>',
				    ...
				    descriptionShowAttr   = '<p class="description">' + woodmartConfig.attribute_show_on_product_description + '</p>',
				    metaHTMLTable         = [
					    //Select swatch size
