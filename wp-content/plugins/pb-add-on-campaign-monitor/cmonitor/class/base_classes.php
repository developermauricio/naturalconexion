...
define('WPPB_CS_REST_WRAPPER_VERSION', '4.0.2');
...
define('WPPB_CS_REST_WEBHOOK_FORMAT_JSON', 'json');
define('WPPB_CS_REST_WEBHOOK_FORMAT_XML', 'xml');
 ...
 *
 */
class WPPB_CS_REST_Wrapper_Result {
    /**
 ...
 *
 */
class WPPB_CS_REST_Wrapper_Base {
    /**
     ...
     * @var WPPB_CS_REST_JsonSerialiser or WPPB_CS_REST_XmlSerialiser
     * @access private
     */
    var $_serialiser;
     ...
     * @var WPPB_CS_REST_CurlTransport or WPPB_CS_REST_SocketTransport or your own custom transport.
     * @access private
     */
    var $_transport;
    ...
    function __construct(
        ...
        $this->_log = is_null($log) ? new WPPB_CS_REST_Log($debug_level) : $log;

        $this->_protocol = $protocol;
        $this->_base_route = $protocol.'://'.$host.'/api/v3.1/';

        $this->_log->log_message('Creating wrapper for '.$this->_base_route, get_class($this), WPPB_CS_REST_LOG_VERBOSE);

        $this->_transport = is_null($transport) ?
            WPPB_CS_REST_TRANSPORT_get_available($this->is_secure(), $this->_log) :
            $transport;

        $transport_type = method_exists($this->_transport, 'get_type') ? $this->_transport->get_type() : 'Unknown';
        $this->_log->log_message('Using '.$transport_type.' for transport', get_class($this), WPPB_CS_REST_LOG_WARNING);

        $this->_serialiser = is_null($serialiser) ?
            WPPB_CS_REST_SERIALISATION_get_available($this->_log) : $serialiser;

        $this->_log->log_message('Using '.$this->_serialiser->get_type().' json serialising', get_class($this), WPPB_CS_REST_LOG_WARNING);

        $this->_default_call_options = array (
            'authdetails' => $auth_details,
            'userAgent' => 'WPPB_CS_REST_Wrapper v'.WPPB_CS_REST_WRAPPER_VERSION.
    ...
    function refresh_token() {
        ...
        $wrap = new WPPB_CS_REST_Wrapper_Base(
            NULL, 'https', WPPB_CS_REST_LOG_NONE, CS_HOST, NULL,
            new WPPB_CS_REST_DoNothingSerialiser(), NULL);
    ...
    function put_request($route, $data, $call_options = array()) {
        return $this->_call($call_options, WPPB_CS_REST_PUT, $route, $data);
    }
    
    function post_request($route, $data, $call_options = array()) {
        return $this->_call($call_options, WPPB_CS_REST_POST, $route, $data);
    }
    
    function delete_request($route, $call_options = array()) {
        return $this->_call($call_options, WPPB_CS_REST_DELETE, $route);
    }
    
    function get_request($route, $call_options = array()) {
        return $this->_call($call_options, WPPB_CS_REST_GET, $route);
    }
    ...
    function _call($call_options, $method, $route, $data = NULL) {
        ...
        $this->_log->log_message('Making '.$call_options['method'].' call to: '.$call_options['route'], get_class($this), WPPB_CS_REST_LOG_WARNING);
            
        $call_result = $this->_transport->make_call($call_options);

        $this->_log->log_message('Call result: <pre>'.var_export($call_result, true).'</pre>',
            get_class($this), WPPB_CS_REST_LOG_VERBOSE);

        if($call_options['deserialise']) {
            $call_result['response'] = $this->_serialiser->deserialise($call_result['response']);
        }
         
        return new WPPB_CS_REST_Wrapper_Result($call_result['response'], $call_result['code']);
    }
}
