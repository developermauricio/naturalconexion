<?php
    ...
    class WPPB_Campaign_Monitor_Widget extends WP_Widget {
        ...
        public function widget( $args, $instance )
        {
            ...
            if (!wppb_cmi_get_api_key_status($wppb_cmi_settings) && current_user_can('manage_options')) {

                echo $args['before_widget'];

                if (!empty($instance['title'])) {
                    echo $args['before_title'] . apply_filters('wpp_cmi_widget_title', $instance['title']) . $args['after_title'];
                }

                echo __('Something went wrong. Either the Campaign Monitor API key is missing or it is invalid.', 'profile-builder');
                ...
                if ($validate_response['success'] == 1) {
                    ...
                    if ($result->was_successful()) {
                        echo '<div class="wppb-cmi-widget-subscribe-success-wrapper">';
                            echo '<p class="wppb-cmi-widget-subscribe-success-message">' . sprintf(__('%s', 'profile-builder'), apply_filters('wppb_cmi_widget_subscribe_new_success', 'You have been subscribed to our newsletter')) . '</p>';
                        echo '</div>';
                    ...
                    if (!empty($instance['cmi_list_id']) && !empty($instance['cmi_list_fields'])) {
                        echo '<form enctype="multipart/form-data" method="POST" action="' . apply_filters('wppb_cmi_widget_subscribe_form_action', '') . '">';

                        // Add hidden inputs with the list ids
                        echo '<input type="hidden" name="wppb_cmi_list_id" value="' . $instance['cmi_list_id'] . '" />';
                        ...
                        foreach ($instance['cmi_list_fields'] as $wpp_cmi_list_field) {
                            ...
                            if (isset($wpp_cmi_list_field['tag']) && !empty($wpp_cmi_list_field['tag'])) {
                                ...
                                // Field wrapper
                                echo '<div class="wppb-cmi-widget-field-wrapper ' . $error_class . '">';


                                // Display field label
                                echo '<label for="wppb_cmi_widget_request_' . $wpp_cmi_list_field['tag'] . '">' . $wpp_cmi_list_field['name'] . '</label>';


                                // Display required span if field is required
                                if (isset($wpp_cmi_list_field['req']))
                                    echo '<span class="wppb-cmi-widget-field-required">' . apply_filters('wppb_cmi_widget_field_required', '*') . '</span>';
                                ...
                                echo '<input type="text" id="wppb_cmi_widget_request_' . $wpp_cmi_list_field['tag'] . '" name="wppb_cmi_widget_request_' . $wpp_cmi_list_field['tag'] . '" value="' . $field_value . '" />';
                                ...
                                if (isset($validate_response['success']) && $validate_response['success'] == 0 && isset($validate_response['fields'][$wpp_cmi_list_field['tag']]) && isset($validate_response['fields'][$wpp_cmi_list_field['tag']]['errors'])) {
                                    foreach ($validate_response['fields'][$wpp_cmi_list_field['tag']]['errors'] as $error) {
                                        echo '<div class="wppb-cmi-widget-field-error-message-wrapper"><span class="wppb-cmi-widget-field-error-message">' . $error . '</span></div>';
                                    }
                                }
                        ...
                        echo '<input type="submit" value="' . apply_filters('wppb_cmi_widget_submit_button_text', __('Submit', 'profile-builder')) . '" />';
        ...
        public function form( $instance ) {
            ...
            if( wppb_cmi_get_api_key_status( $wppb_cmi_settings ) ) {
                ...
                <div class="wppb-cmi-widget-wrapper">
                    ...
                    <label class="wppb-cmi-widget-label" for="<?php echo $this->get_field_id('title'); ?>"><?php _e('Title', 'profile-builder'); ?></label>

                    <div class="wppb-cmi-widget-input-wrapper">
                        <input class="widefat" id="<?php echo $this->get_field_id('title'); ?>" name="<?php echo $this->get_field_name('title'); ?>" type="text" value="<?php echo esc_attr($title); ?>">
                    </div>

                    <label class="wppb-cmi-widget-label" for="<?php echo $this->get_field_id('cmi_list_id'); ?>"><?php _e( 'Select the list your new subscriber will be added to', 'profile-builder' ); ?></label>

                    <div class="wppb-cmi-widget-input-wrapper">
                        <select id="<?php echo $this->get_field_id('cmi_list_id'); ?>" name="<?php echo $this->get_field_name('cmi_list_id') ?>" class="widefat">
                            <?php if( !empty($wppb_cmi_settings['client']['lists']) ): ?>
                                <option value=""><?php echo __( 'Select list...', 'profile-builder' ); ?></option>

                                <?php foreach( $wppb_cmi_settings['client']['lists'] as $wppb_cmi_list_id => $wppb_cmi_list ): ?>
                                    <option <?php selected( $cmi_list_id, $wppb_cmi_list_id, true); ?> value="<?php echo $wppb_cmi_list_id; ?>"><?php echo $wppb_cmi_list['name']; ?></option>
                                <?php endforeach; ?>
                            <?php else: ?>
                                <option value=""><?php echo __( 'No lists found', 'profile-builder' ); ?></option>
                            <?php endif; ?>
                        </select>
                    </div>

                    <label class="wppb-cmi-widget-label"><?php _e( 'Select which fields to show', 'profile-builder' ); ?></label>

                    <div class="wppb-cmi-widget-input-wrapper">
                        <label class="wppb-cmi-widget-checkbox-label">
                            <input type="hidden" value="email" name="<?php echo $this->get_field_name('cmi_list_fields'); ?>[email][tag]" />
                            <input type="checkbox" value="" disabled="disabled" checked="checked" />
                            <?php echo __( 'E-mail address', 'profile-builder' ); ?>
                        </label>
                        <input type="hidden" value="<?php echo __( 'E-mail address', 'profile-builder' ); ?>" name="<?php echo $this->get_field_name('cmi_list_fields'); ?>[email][name]" />
                        <input type="hidden" value="1" name="<?php echo $this->get_field_name('cmi_list_fields'); ?>[email][req]" />

                        <label class="wppb-cmi-widget-checkbox-label">
                            <input type="checkbox" value="<?php echo __( 'fullname', 'profile-builder' ); ?>" name="<?php echo $this->get_field_name('cmi_list_fields'); ?>[fullname][tag]" <?php checked( $cmi_list_field_fullname, 'fullname', true); ?> />
                            <?php echo __( 'Name', 'profile-builder' ); ?>
                        </label>
                        <input type="hidden" value="<?php echo __( 'Name', 'profile-builder' ); ?>" name="<?php echo $this->get_field_name('cmi_list_fields'); ?>[fullname][name]" />
                    </div>

                    <label class="wppb-cmi-widget-label"><?php _e( 'Extra options', 'profile-builder' ); ?></label>

                    <div class="wppb-cmi-widget-input-wrapper">
                        <label class="wppb-cmi-widget-checkbox-label">
                            <input type="checkbox" name="<?php echo $this->get_field_name('cmi_list_resubscribe'); ?>" <?php checked( $cmi_list_resubscribe, 'on', true); ?> />
                            <?php echo __( 'Resubscribe', 'profile-builder' ); ?>
                        </label>

                        <p class="description"><?php echo __( 'If the subscriber is in an inactive state or has previously been unsubscribed and you check the Resubscribe option, they will be re-added to the list. Therefore, this method should be used with caution and only where suitable.', 'profile-builder' ); ?></p>
                    </div>
                </div>

            <?php
            } else {
                echo '<p>' . __( 'The Campaign Monitor API key is either missing or is invalid.', 'profile-builder' ) . '</p>';
            }
        }
        ...
        public function validate_front_end_form() {
            ...
            if( isset( $_POST['wppb_cmi_list_id'] ) && !empty( $_POST['wppb_cmi_list_id'] ) )
                $response['subscribe']['list_id'] = htmlspecialchars(trim($_POST['wppb_cmi_list_id']));
            else
                return false;
            ...
            foreach( $instance['cmi_list_fields'] as $wpp_cmi_list_field ) {
                ...
                if( isset( $wpp_cmi_list_field['tag'] ) && isset( $_POST['wppb_cmi_widget_request_' . $wpp_cmi_list_field['tag']] ) ) {
                    ...
                    $field_value = htmlspecialchars(trim( $_POST['wppb_cmi_widget_request_' . $wpp_cmi_list_field['tag']] ) );

                    if( !empty( $field_value ) ) {

                        // E-mail field
                        if( $wpp_cmi_list_field['tag'] == 'email' && !is_email( $field_value ) ) {
                            $response_error['fields'][ $wpp_cmi_list_field['tag'] ]['errors'][] = apply_filters( 'wppb_cmi_widget_field_error_email_not_valid', 'The e-mail is not valid' );
                            $error = 1;
    ...
    add_action( 'widgets_init', function(){
        register_widget( 'WPPB_Campaign_Monitor_Widget' );
    });