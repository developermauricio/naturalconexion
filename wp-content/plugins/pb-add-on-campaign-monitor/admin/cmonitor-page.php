<?php
     ...
     * @return void
     */
    function wppb_cmi_register_submenu_page() {
        add_submenu_page( 'profile-builder', __( 'Campaign Monitor', 'profile-builder' ), __( 'Campaign Monitor', 'profile-builder' ), 'manage_options', 'profile-builder-campaign-monitor', 'wppb_cmi_page_content' );
    }
    add_action( 'admin_menu', 'wppb_cmi_register_submenu_page', 20 );
     ...
     * @return string
     */
    function wppb_cmi_page_content() {
        ...
        <div class="wrap wppb-wrap wppb-cmi-wrap">
            <form method="post" action="options.php">
                ...
                <?php
                    ...
                    // Page title
                    echo '<h2>' . __( 'Campaign Monitor Integration', 'profile-builder' ) . '</h2>';

                    // Display the API key settings input
                    echo wppb_cmi_get_page_api_key_content( $wppb_cmi_settings );

                    // If API key is okay display the rest of the information
                    if( wppb_cmi_get_api_key_status( $wppb_cmi_settings ) ) {

                        // Display the client selector
                        if( !isset( $wppb_cmi_settings['client'] ) )
                            echo wppb_cmi_get_page_client_selector_content( $wppb_cmi_settings, false );
                        else
                            echo wppb_cmi_get_page_client_selector_content( $wppb_cmi_settings );

                        // Display the clients and each of their lists
                        echo wppb_cmi_get_page_clients_settings_content( $wppb_cmi_settings );
                    }

                ?>

                <input id="wppb-cmi-page-submit" type="submit" class="button button-primary" value="<?php echo __( 'Save', 'profile-builder' ); ?>" />
     ...
     *
     */
    function wppb_cmi_get_page_api_key_content( $wppb_cmi_settings ) {
        ...
        $output = '<div id="wppb-cmi-page-api-key">';
            ...
            if( !empty( $wppb_cmi_api_key ) && $wppb_cmi_api_key_validated == true ) {
                $output .= '<img src="' . WPPBCMI_PLUGIN_URL . '/assets/img/icon_okay.png" title="' . __( 'The API key was successfully validated!', 'profile-builder' ) . '"/>';
            ...
            // Display the error icon if the API key has not been validated
                ...
                $output .= '<img src="' . WPPBCMI_PLUGIN_URL . '/assets/img/icon_error.png" title="' . __( 'Either the API key is not valid or we could not connect to Campaign Monitor to validate it!', 'profile-builder' ) . '"/>';
            }
     ...
     *
     */
    function wppb_cmi_get_page_client_selector_content( $wppb_cmi_settings, $is_client = true ) {
            ...
            $output .= '<div class="wppb-cmi-page-client-selector-select-wrapper">';
                ...
                $output .= '<select id="wppb-cmi-page-client-selector-select" class="widefat">';
                    ...
                    // If we don't have a client saved in the db display all clients options
                        ...
                        $output .= wppb_cmi_page_client_selector_options_content( $wppb_cmi_settings );
                    }
     ...
     *
     */
    function wppb_cmi_page_client_selector_options_content( $wppb_cmi_settings = array() ) {
        ...
        $wrap = new WPPB_CS_REST_General($auth);
        ...
        if( isset($result) && $result->was_successful() ) {
            ...
            if( empty( $result->response ) )
                $output .= '<option value="">' . __( 'No clients found', 'profile-builder' ) . '</option>';
            else
                $output .= '<option value="">' . __( 'Select a client...', 'profile-builder' ) . '</option>';

            $clients = $result->response;

            foreach ($clients as $client) {
                $output .= '<option value="' . $client->ClientID . '">' . $client->Name . '</option>';
            }
        }
    ...
    add_action( 'wp_ajax_wppb_cmi_page_client_selector_options_content', 'wppb_cmi_page_client_selector_options_content' );
     ...
     *
     */
    function wppb_cmi_get_page_clients_settings_content( $wppb_cmi_settings ) {
        ...
        $output = '<div id="wppb-cmi-page-clients-settings-wrapper">';
            ...
            if( isset( $wppb_cmi_settings['client'] ) && !empty( $wppb_cmi_settings['client'] ) ) {
                ...
                $output .= wppb_cmi_get_page_client_lists_table_content( $wppb_cmi_settings['client']['id'], $wppb_cmi_settings );
     ...
     *
     */
    function wppb_cmi_get_page_client_lists_table_content( $wppb_cmi_client_id, $wppb_cmi_settings = array() ) {
        ...
        if( isset( $_POST['wppb_cmi_client_id'] ) ) {
            $is_ajax = true;
            $wppb_cmi_client_id = trim( $_POST['wppb_cmi_client_id'] );
        }
        ...
        $output = '<table id="wppb-cmi-list-table-client-data" class="wp-list-table widefat fixed posts wppb-cmi-list-table">';
            ...
            // Client name
            $output .= wppb_cmi_get_page_client_lists_caption_content( $wppb_cmi_client_id, $wppb_cmi_settings, $is_ajax );

            // Table head
            $output .= '<thead>';
                $output .= '<tr>';
                    $output .= '<th scope="col" class="manage-column column-title" style="width: 80%;"><span>' . __( 'Client List','profile-builder' ) . '</span></th>';
                    $output .= '<th scope="col" class="manage-column column-title"><span>' . __( 'Fields Count','profile-builder' ) . '</span></th>';
                $output .= '</tr>';
            $output .= '</thead>';
            ...
            $output .= '<tfoot>';
                $output .= '<tr>';
                    $output .= '<th scope="col" class="manage-column column-title""><span>' . __( 'Client List','profile-builder' ) . '</span></th>';
                    $output .= '<th scope="col" class="manage-column column-title"><span>' . __( 'Fields Count','profile-builder' ) . '</span></th>';
                $output .= '</tr>';
            $output .= '</tfoot>';
            ...
            // Table body
            $output .= '<tbody>';
                $output .= wppb_cmi_get_page_client_lists_tbody_content( $wppb_cmi_client_id, $wppb_cmi_settings, $is_ajax );
            $output .= '</tbody>';
    ...
    add_action( 'wp_ajax_wppb_cmi_get_page_client_lists_table_content', 'wppb_cmi_get_page_client_lists_table_content' );
     ...
     *
     */
    function wppb_cmi_get_page_client_lists_caption_content( $wppb_cmi_client_id, $wppb_cmi_settings = array(), $is_ajax ) {
        ...
        if( $is_ajax ) {
            ...
            $auth = array( 'api_key' => $wppb_cmi_settings['api_key'] );
            $wrap = new WPPB_CS_REST_Clients( $wppb_cmi_client_id, $auth );
            $result = $wrap->get();
        ...
        $output .= '<caption class="wppb-cmi-table-caption">';
            ...
            $output .= '<input class="wppb-cmi-client-id" type="hidden" name="wppb_cmi_settings[client][id]" value="' . $wppb_cmi_client_id . '" />';
            $output .= '<input class="wppb-cmi-client-id" type="hidden" name="wppb_cmi_settings[client][name]" value="' . $wppb_cmi_client_name . '" />';

            // Options buttons for client
            $output .= '<a id="wppb-cmi-sync-client-btn" href="#" class="add-new-h2 alignright" title="' . __( 'Retrieves changes made in your Campaign Monitor account and matches it with the saved data from the add-on. This does not save the new data, so you will have to manually save.', 'profile-builder' ) . '">' . __( 'Synchronize client data', 'profile-builder' ) . '</a>';
            $output .= '<a id="wppb-cmi-change-client-btn" href="#" class="add-new-h2 alignright">' . __( 'Change client', 'profile-builder' ) . '</a>';
     ...
     *
     */
    function wppb_cmi_get_page_client_lists_tbody_content( $wppb_cmi_client_id, $wppb_cmi_settings = array(), $is_ajax ) {
        ...
        if( !empty( $wppb_cmi_client_lists ) ) {
            foreach( $wppb_cmi_client_lists as $wppb_cmi_client_list ) {
                $output .= wppb_cmi_get_page_client_list_row_content( $wppb_cmi_client_list, $wppb_cmi_settings, $is_ajax );
            }

        // If no lists found return message that no lists were found
        } else {
            $output .= '<tr><td>' . __( 'No lists were found.', 'profile-builder' ) . '</td></tr>';
        }
     ...
     *
     */
    function wppb_cmi_get_page_client_list_row_content( $wppb_cmi_list, $wppb_cmi_settings = array(), $is_ajax = false ) {

        // Row with the list name, field count
        $output = '<tr class="wppb-cmi-list">';

            $output .= '<td class="post-title column-title">';
                $output .= '<strong><a class="wppb-cmi-list-edit" href="#" title="' . __( 'Click to edit', 'profile-builder' ) . '">' . $wppb_cmi_list->Name . '</a></strong>';
                $output .= '<input type="hidden" name="wppb_cmi_settings[client][lists][' . $wppb_cmi_list->ListID . '][name]" value="' . $wppb_cmi_list->Name . '" />';
            $output .= '</td>';
        ...
        $output .= '<tr class="wppb-cmi-list-settings hidden">';
            $output .= '<td>';
                $output .= wppb_cmi_get_page_client_list_fields_content( $wppb_cmi_list, $wppb_cmi_settings, $is_ajax );
                $output .= wppb_cmi_get_page_client_list_extra_options_content( $wppb_cmi_list, $wppb_cmi_settings );

                $output .= '<a href="#" class="wppb-cmi-list-settings-cancel button">' . __( 'Cancel', 'profile-builder' ) . '</a>';
            $output .= '</td>';
        $output .= '</tr>';
     ...
     *
     */
    function wppb_cmi_get_page_client_list_fields_content( $wppb_cmi_list, $wppb_cmi_settings, $is_ajax = false ) {

        $wppb_cmi_fields = array();
        $wppb_manage_fields = wppb_cmi_get_manage_fields();
        ...
        if( $is_ajax ) {
            ...
            // Connect to Campaign Monitor
            $auth = array( 'api_key' => $wppb_cmi_settings['api_key'] );
            $wrap = new WPPB_CS_REST_Lists( $wppb_cmi_list->ListID, $auth );
        ...
        $output = '<div class="wppb-cmi-list-settings-section">';
            ...
            // Section title
            $output .= '<strong class="wppb-cmi-list-settings-section-title">' . __( 'Field Associations:', 'profile-builder' ) . '</strong>';
            ...
            foreach( $wppb_cmi_fields as  $wppb_cmi_field ) {
                ...
                $output .= '<label for="wppb-cmi-list-field-' . $wppb_cmi_list->ListID . '-' . $wppb_cmi_field_key . '">' . $wppb_cmi_field->FieldName . '</label>';

                // Output a hidden field to save the field name
                $output .= '<input type="hidden" name="wppb_cmi_settings[client][lists][' . $wppb_cmi_list->ListID . '][fields][' . $wppb_cmi_field_key . '][name]" value="' . $wppb_cmi_field->FieldName . '" />';

                // Output the drop down with the PB manage fields
                $output .= '<select id="wppb-cmi-list-field-' . $wppb_cmi_list->ListID . '-' . $wppb_cmi_field_key . '" name="wppb_cmi_settings[client][lists][' . $wppb_cmi_list->ListID . '][fields][' . $wppb_cmi_field_key . '][request_name]">';
                ...
                if($wppb_cmi_field->Key == 'email'){
		            foreach ($wppb_manage_fields as $wppb_field) {
			            if($wppb_field['field'] == 'Default - E-mail' || ($wppb_field['field'] == 'Email') ){
				            // Get request name
				            $request_name = wppb_cmi_get_request_name($wppb_field);
				            ...
				            $output .= '<option ' . $selected . ' value="' . $request_name . '">' . $wppb_field['field-title'] . ' ( ' . $wppb_field['field'] . ' )' . '</option>';
			            }
		            }
	            } else {
		            $output .= '<option value="">' . __('None', 'profile-builder') . '</option>';
		            foreach ($wppb_manage_fields as $wppb_field) {
			            // Get request name
			            $request_name = wppb_cmi_get_request_name($wppb_field);
            ...
            $output .= '<p class="description">' . __( 'Associate each Campaign Monitor field with a Profile Builder field', 'profile-builder' ) . '</p>';
        $output .= '</div>';
     ...
     *
     */
    function wppb_cmi_get_page_client_list_extra_options_content( $wppb_cmi_list, $wppb_cmi_settings ) {

        // Start outputing information
        $output = '<div class="wppb-cmi-list-settings-section">';

            // Section title
            $output .= '<strong class="wppb-cmi-list-settings-section-title">' . __( 'Extra Options:', 'profile-builder' ) . '</strong>';
            ...
            // Output option input
            $output .= '<label><input type="checkbox" name="wppb_cmi_settings[client][lists][' . $wppb_cmi_list->ListID . '][resubscribe]" ' . $checked . ' />' . __( 'Resubscribe', 'profile-builder' ) . '</label>';

            // Ouput option description
            $output .= '<p class="description">' . __( 'If the subscriber is in an inactive state or has previously been unsubscribed and you check the Resubscribe option, they will be re-added to the list. Therefore, this method should be used with caution and only where suitable.', 'profile-builder' ) . '</p>';
        $output .= '</div>';
     ...
     *
     */
    function wppb_cmi_settings_sanitize( $wppb_cmi_settings_new ) {
    ...
    * @since v.1.0.0
    */
    function wppb_cmi_settings_admin_notices() {
        settings_errors( 'wppb_cmi_settings_error' );
    }
    add_action( 'admin_notices', 'wppb_cmi_settings_admin_notices' );