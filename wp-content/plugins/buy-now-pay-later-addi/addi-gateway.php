...
function addi_before_add_to_cart_form($argPosition){
	...
	$styles_to_json = "";
    $table_config_name = $wpdb->prefix . "wc_addi_config";
    $result = $wpdb->get_results($wpdb->prepare("select * from {$table_config_name} where element = 'widget'"));
    $position = "";

    $resultV = $wpdb->get_results($wpdb->prepare("select * from {$table_config_name} where element = 'widget_position'"));
        ...
        if(isset($result) && count($result) > 0) {
            ...
            $modalButtonFontColor_ = '';
                ...
                foreach ($result as $item) {
                    ...
                    $conf_result = $wpdb->get_results($wpdb->prepare("select * from {$table_config_name} where element like 'conf_%'"));
...
function wpdocs_my_enqueue_scripts() {
    ...
    add_filter( 'script_loader_tag', __NAMESPACE__ . '\wpdocs_my_add_attributes', 10, 2 );
}

/**
* Add attributes based on defined script/style handles.
*/
function wpdocs_my_add_attributes( $html, $handle ) : string {
	//echo "<h3> " . $handle . "</h3>";
    global $wpdb;
    $table_config_name = $wpdb->prefix . "wc_addi_config";
    ...
    switch( $handle ) {
        case 'widget-addi':
            $result = $wpdb->get_results($wpdb->prepare("select * from {$table_config_name} where element = 'widget_home'"));
            if(isset($result) && count($result) > 0) {
                foreach ($result as $item) {
                    $newValue = $item->value;
                    $split = explode("|",$newValue);
                ...
                $resultW = $wpdb->get_results($wpdb->prepare("select * from {$table_config_name} where element = 'widget'"));
...
function woocommerce_available_payment_gateways( $available_gateways ) {
    ...
    if (array_key_exists('addi',$available_gateways)) {
		 if((get_locale() == 'pt_PT' || get_locale() == 'pt_BR')) {
			 $available_gateways['addi']->order_button_text = __( 'Ir para pagar', 'woocommerce' );
		 }else {
...
function addi_init_gateway_class() {   
	...
	class WC_Addi_Gateway extends WC_Payment_Gateway {
 		...
 		public function __construct() {
            ...
            add_action('update_option', function( $option_name, $old_value, $value ) {
                ...
                if($option_name == 'woocommerce_addi_settings') {
                    ...
                    $resultH = $wpdb->get_results($wpdb->prepare("select * from {$table_config_name} where element = 'widget_home'"));
		...
		public function payment_fields() {
            ...
            if((get_locale() == 'pt_PT' || get_locale() == 'pt_BR')) {
                $api_app_url = 'https://channels-public-api.addi.com.br/allies/' . $this->widget_slug .'/config?requestedAmount=' . $totals;
            }
            else {
                $api_app_url = 'https://channels-public-api.addi.com/allies/' . $this->widget_slug .'/config?requestedAmount=' . $totals;
            }
			...
			if( !is_wp_error( $api_response ) ) { 
			 ...
			 //print_r(array_values($body_api_response));
			}

            $description = "<div class='addi_description_container' style='background-image: url(" . plugins_url( '/assets/bgone.png' , __FILE__ ) . ");background-size: 100%;background-repeat: no-repeat;max-width:360px;border: 1px solid #C9CDD1; box-sizing: border-box; border-radius: 8px;'>";

            if((get_locale() == 'pt_PT' || get_locale() == 'pt_BR')) {
				$widgetversion = $body_api_response['widgetConfig']['widgetVersion'];
				
				if(isset($widgetversion) && ($widgetversion === '1.0.1' || $widgetversion === 'ADDI_TEMPLATE_01')) {
				
			    ?>
                    <script type="text/javascript">
                        jQuery(document).ready( function($) {
                            const get_payment_method = $('li.wc_payment_method.payment_method_addi > label');
                            if (get_payment_method) {
                                get_payment_method.text('PIX parcelado ');
                                get_payment_method.css("display", "inline-flex");
                                get_payment_method.append('<img style="margin-left: 6px !important;margin-top: -1px; !important; max-height: 20px !important;" src="<?php echo $this->icon ?>" alt="PIX parcelado">');
                            };
                            });
                    </script>
                    
                <?php
                $description = $description . "<div style='height: 50px;border-bottom: 1px solid #C9CDD1;background: #091A42; border-radius: 8px 8px 0 0;color: white;font-size: 14px;line-height: 19.74px;padding: 7px 30px 0 18px;'><span>Após clicar em &#8220;ir para pagar&#8221;, você será redirecionado para Addi.</span></div><div class='container' style='padding: 20px;'><div style='margin-bottom: 10px;'><img style='margin-bottom: 11px;' src='" . plugins_url( '/assets/title_br.png' , __FILE__ ) ."'></div><div class='mini-frame' style='border-radius: 8px;border: 1px solid #EBECED;padding: 8px;box-sizing: border-box;background-color: white;display: inline-flex;'><span style='font-weight: 500;font-size: 14px;'>Com <b>Addi</b> você parcela suas compras em 1 + 3x no <b>PIX, sem juros!*</b></br>Você só precisa do Pix e seu WhatsApp</span></div><div style='margin-top: 10px;    line-height: 20px;'><i style='font-size: small;'>*O pagamento precisa ter o mesmo CPF da compra.</i></div></div>";
                
                }
                elseif (isset($widgetversion) && ($widgetversion === '1.0.2' || $widgetversion === 'ADDI_TEMPLATE_02')) {
                ...
                ?>
                    <script type="text/javascript">
                        jQuery(document).ready( function($) {
                            const get_payment_method = $('li.wc_payment_method.payment_method_addi > label');
                            if (get_payment_method) {
                                get_payment_method.text('PIX à vista ou parcelado ');

                                get_payment_method.css("display", "inline-flex");
                                get_payment_method.append('<img style="margin-left: 6px !important;margin-top: -1px; !important; max-height: 20px !important;" src="<?php echo $this->icon ?>" alt="PIX à vista ou parcelado">');
                            };
                            });
                    </script>
                    
                <?php
                $description = $description . "<div style='height: 50px;border-bottom: 1px solid #C9CDD1;background: #091A42; border-radius: 8px 8px 0 0;color: white;font-size: 14px;line-height: 19.74px;padding: 7px 30px 0 18px;'><span>Após clicar em &#8220;ir para pagar&#8221;, você será redirecionado para Addi.</span></div><div class='container' style='padding: 20px;'><div style='margin-bottom: 10px;'><img style='margin-bottom: 11px;' src='" . plugins_url( '/assets/title_br.png' , __FILE__ ) ."'></div><div class='mini-frame' style='border-radius: 8px;border: 1px solid #EBECED;padding: 8px;box-sizing: border-box;background-color: white;display: inline-flex;'><span style='font-weight: 500;font-size: 14px;margin-bottom: -22px;'>Com a <b>Addi</b> você pode escolher a melhor opção para <b>pagar com pix:</b></p><ul style='margin-top: -18px;padding-left:15px;'><li style='list-style-type: circle;list-style: outside; margin-bottom:2px;'>PIX à vista!</li><li style='list-style-type: circle;list-style: outside;margin-bottom:2px;'>1 + 3x no PIX, sem juros!*</li></ul><div>Você só precisa do Pix e seu WhatsApp</div></span></div><div style='margin-top: 10px;    line-height: 20px;'><i style='font-size: small;'>*O pagamento precisa ter o mesmo CPF da compra.</i></div></div>";
                    ...
                    <script type="text/javascript">
                        jQuery(document).ready( function($) {
                            ...
                            if (get_payment_method) {
                                get_payment_method.text('PIX parcelado ');
                                get_payment_method.css("display", "inline-flex");
                                get_payment_method.append('<img style="margin-left: 6px;margin-top: -1px;" src="<?php echo $this->icon ?>" alt="PIX parcelado">');
                            };
                            });
                    </script>
			...
			if( !is_wp_error( $api_response ) ) { 
			 ...
			 if(isset($body_api_response['policy']['discount']) &&
                 $body_api_response['policy']['discount'] !== "" && 
                 $body_api_response['policy']['discount'] !== 0 && 
                 $body_api_response['policy']['discount'] !== "0") {
                if((get_locale() == 'pt_PT' || get_locale() == 'pt_BR')) {
                    $description = $description . "<div class='discount-container'><span class='discount-label'>" .
                    $body_api_response['policy']['discount'] * 100 . "% do desconto</span></div>
                    <div class='discount-sub-container'><span class='discount-sub-label'>*Desconto será aplicado direto no carrinho após selecionar &#8220;Ir para pagar&#8221;</span></div>";
                }
                else {
                    $description = $description . "<div class='discount-container'><span class='discount-label'>" .
                    $body_api_response['policy']['discount'] * 100 . "% de descuento</span></div>
            ...
            if( !is_wp_error( $api_response ) ) { 
				if(isset($body_api_response['maxAmount']) &&
                   $body_api_response['maxAmount'] !== "" &&
				   isset($body_api_response['minAmount']) &&
                   $body_api_response['minAmount'] !== "") {
					if($totals < intval($body_api_response['minAmount'])) {
						if((get_locale() == 'pt_PT' || get_locale() == 'pt_BR')) {
                            $description = $description . "<div class='constraint-container'>Disponível somente para compras acima de R$" .
                            number_format(intval($body_api_response['minAmount']),0,',','.') . "</div>";
						}
						else {
					...
					if ($totals > intval($body_api_response['maxAmount'])) {
						if((get_locale() == 'pt_PT' || get_locale() == 'pt_BR')) {
                           $description = $description . "<div class='constraint-container'>Disponível somente para compras abaixo de R$" .
                           number_format(intval($body_api_response['maxAmount']),0,',','.') . "</div>";
						}
						else{
				   ...
				   if(isset($body_api_response['code']) && $body_api_response['code'] == '007-015') {
					   if((get_locale() == 'pt_PT' || get_locale() == 'pt_BR')) {
                          $description = $description . "<div class='constraint-container'>Disponível somente para compras abaixo de R$" .
                          number_format(intval($totals),0,',','.') . "</div>";
						}
						else{
                          $description = $description . "<div class='constraint-container'>Solo disponible para compras menores a $" .
                          number_format(intval($totals),0,',','.') . "</div>";
        ...
        public function process_payment( $order_id ) {
            ...
            // verify if boy response is an error or contains data
            if( !is_wp_error( $auth_response ) ) {   
                ...
                */
                ...
                if( !is_wp_error( $online_application_response ) ) { 
