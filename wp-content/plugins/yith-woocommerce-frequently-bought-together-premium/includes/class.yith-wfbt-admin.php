...
if ( ! class_exists( 'YITH_WFBT_Admin' ) ) {
	...
	class YITH_WFBT_Admin {
		...
		public function action_links( $links ) {
			$links = yith_add_action_links( $links, $this->_panel_page, true, YITH_WFBT_SLUG );
			return $links;
		}
			...
			<td class="forminp yith_image_size_settings">
				<div class="yith_image_size_wrap">
					...
					<label><input name="<?php echo esc_attr( $value['id'] ); ?>[crop]"
							...
							value="1" <?php checked( 1, $crop ); ?> /> <?php _e( 'Hard Crop?', 'yith-woocommerce-frequently-bought-together' ); ?></label>
				</div>
